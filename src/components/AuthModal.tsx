...
export default function AuthModal({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (authMode === 'login') {
      const match = registeredList.find(u => u.email.toLowerCase() === cleanEmail && u.password === password);
      
      if (match) {
        setFeedback({ type: 'success', text: match.role === 'admin' ? t.auth_success_admin : t.auth_success_customer });
        setTimeout(() => {
          onAuthSuccess(match);
          onClose();
        }, 1200);
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4">
      ...
      <div id="auth-form-dialog" className="relative bg-white dark:bg-[#11141D] rounded-3xl w-full max-w-md overflow-hidden shadow-2xl p-6 sm:p-8 border border-slate-150 dark:border-[#1E293B] animate-in fade-in zoom-in-95 duration-200 text-slate-800 dark:text-gray-100">
          ...
          {authMode === 'register' && (
            <div className="space-y-1">
              ...
              <input
                ...
                className={`w-full text-xs px-3.5 py-3 rounded-xl border bg-slate-50 dark:bg-[#0A0C10] border-transparent focus:border-[var(--primary-color, #38bdf8)] focus:bg-white dark:focus:bg-black text-slate-800 dark:text-white outline-none transition-all ${
          ...
          <div className="space-y-1">
            ...
            <div className="relative">
              ...
              <input
                ...
                className={`w-full text-xs py-3 px-3.5 pr-10 rounded-xl border bg-slate-50 dark:bg-[#0A0C10] border-transparent focus:border-[var(--primary-color, #38bdf8)] focus:bg-white dark:focus:bg-black text-slate-850 dark:text-white outline-none transition-all ${
            ...
            <div className="space-y-1 font-sans">
              ...
              <div className="relative font-sans">
                ...
                <input
                  ...
                  className="w-full text-xs py-3 px-10 rounded-xl border bg-slate-50 dark:bg-[#0A0C10] border-transparent focus:border-[var(--primary-color, #38bdf8)] focus:bg-white dark:focus:bg-black text-slate-850 dark:text-white outline-none transition-all placeholder-slate-400 text-center"
                />
                <button
                  type="button"
        ...
        {authMode === 'login' && (
          <div className="mt-6 p-4 rounded-2xl bg-[var(--primary-color, #38bdf8)]/5 border border-[var(--primary-color, #38bdf8)]/10 text-[10px] font-bold text-[var(--primary-color, #38bdf8)] space-y-1 relative">
            <div className="flex items-start gap-1.5 leading-relaxed">
              <ShieldAlert className="w-4 h-4 flex-shrink-0 text-[var(--primary-color, #38bdf8)]" />
              <span>{t.admin_login_hint}</span>
            </div>
          </div>
        )}
